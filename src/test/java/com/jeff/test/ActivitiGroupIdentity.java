...
public class ActivitiGroupIdentity {
         ...
         @Test
         public void deploy(){
        	 Deployment deployment=repositoryService.createDeployment().addClasspathResource("diagrams/helloIdentityGroup.bpmn").name("组任务分配").deploy();
        	...
        	identityService.saveGroup(new GroupEntity("总经理"));
        	 //创建用户
        	identityService.saveUser(new UserEntity("小龙"));
        	identityService.saveUser(new UserEntity("小虎"));

        	identityService.saveUser(new UserEntity("小狼"));
        	identityService.saveUser(new UserEntity("小熊"));
        	//创建关联关系
        	identityService.createMembership("小龙", "部门经理");
        	identityService.createMembership("小虎", "部门经理");
        	identityService.createMembership("小狼", "部门经理");
        	identityService.createMembership("小熊", "总经理");
        	 System.out.println("完成组织机构的构建！！！");
         }
         /**
          * 启动一个请假流程
          */
         ...
         public void start(){
        	 //for(int i=0;i<5;i++){
        		 String processId=runtimeService.startProcessInstanceByKey("helloIdentityGroup").getId();
         ...
         public void  findTask(){
        	 List<Task> list=	 taskService.createTaskQuery().taskCandidateUser("小龙").list();
        	 if(!list.isEmpty()){
        		 for(Task li:list){
